...
# Jds
#
# Goal --> Remove the CSS stuff that Claude wrote, go back to Hunter's
#          code for textual layout
#
#
###############################################################################
...
class Tui000(App):
    ...
    async def on_mount(self) -> None:
        """Set up the application after initial mount."""
        # Initial setup
        await self.question_box.display_question(self.question, self.choices)
    ...
    async def action_debug(self) -> None:
        if not self.debug_open:
            ...
            await self.debug.update(debug_content)
            self.debug_open = True
        else:
            # Hide the debug view
            await self.debug.update("")
